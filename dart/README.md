...
>
> Extracted from an internal Flutter app (R019 / R020) into an independent repo so it can be reused across multiple products (launcher, future Flutter apps) without coupling to any single business package.
> 从某内部 Flutter 应用（R019 / R020）抽离为独立仓库，可在多产品（launcher、未来的 Flutter 应用）间复用，不耦合任何单一业务包。
...
- `0.1.0` — initial release extracted from an internal Flutter app (R019/R020). 从某内部 Flutter 应用（R019/R020）抽离的首发版本。
...
Extracted from an internal app's debug control plane (R019 three-layer scaffold + R020 MCP bridge; the in-tree package was extracted and renamed by R021).
抽自某内部应用的调试控制平面（R019 三层骨架 + R020 MCP 桥；树内包由 R021 抽离并改名）。
