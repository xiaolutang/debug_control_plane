...
**Version alignment / 版本对齐**: `0.5.1` is the latest fully aligned set
...
combination.
**版本对齐**：`0.5.1` 是 kotlin（JitPack）/ dart（pub.dev）/ flutter 插件
...
Current aligned release: `0.5.1`. API unstable (expect breaking changes without
a major bump until the first stable release).
当前对齐版本：`0.5.1`。API 不稳定（在首个稳定版之前，次版本号变更也可能含破坏性改动）。
